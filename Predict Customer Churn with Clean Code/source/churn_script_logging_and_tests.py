...
def df_churn(df):
	# Create 'Churn' column
	df_churn = df.copy()
	df_churn[TARGET_COLUMN] = df_churn['Attrition_Flag'].apply(lambda val: 0 if val == "Existing Customer" else 1)
...
def test_encoder_helper(df_encoded):
	...
	try:
		...
		logging.error(f"Testing encoder_helper: Column {new_col} was not created.")
		raise err
	...
	try:
		...
		logging.error(f"Testing encoder_helper: Column {new_col} contains null values.")
		raise err
	
	logging.info("Testing encoder_helper: New columns created with sound values.")
...
def test_perform_feature_engineering(df_featurized):
	...
	X_train, X_test, y_train, y_test = perform_feature_engineering(df_featurized, response=TARGET_COLUMN)
	...
	try:
		...
		logging.error("Testing perform_feature_engineering: One of the returned sets is empty")
		raise err
	
	logging.info("Testing perform_feature_engineering: Returned sets are not empty")

    # Check if the shapes match
	try:
		assert X_train.shape[1] == X_test.shape[1]
	except AssertionError as err:
		logging.error("Testing perform_feature_engineering: Mismatch in number of features between X_train and X_test.")
		raise err
	
	logging.info("Testing perform_feature_engineering: Sets successfully returned by function.")
...
def test_train_models(df_featurized):
	...
	try:
		...
		logging.error(f"Testing train_models: One or more models were not created.")
		raise err
